...
% Supervisor feedback: no real DOI has been assigned yet, so the class's
% auto-generated placeholder DOI link in the page footer (\rfoot, set from
% \@doinum) should not be shown. Blank it out here, after the class has
% already set it, rather than editing the vendor class file directly.
\rfoot{}
...
\Author{Abdul Rehman *, Raja Ujjan and Muhsin Hassanu *}

\AuthorNames{Abdul Rehman, Raja Ujjan, and Muhsin Hassanu}

\address[1]{Department of Computing and Cybersecurity, University of the West of Scotland, London Campus, London E14 2BA, UK}

\corres{Correspondence: abdulrehmankhokhar25@gmail.com (A.R.)}

\abstract{Web server logs are one of the richest sources of security-relevant information available to defenders, but signature- and rule-based inspection cannot catch attacks that do not match a known pattern, and most existing anomaly-detection studies are validated on a single dataset, log format, and traffic mix, which risks overstating how well a method will generalise once deployed. This paper presents a hybrid anomaly detection framework for web server logs that fuses a content-based autoencoder, which scores individual HTTP requests on their structural features, with a session-based variational LSTM autoencoder, which scores genuine per-client request sequences rather than an arbitrary sliding window over the raw log stream. The framework is evaluated across eight datasets spanning three log formats -- self-generated Apache-style logs, a structured Nginx JSON log, and the public CSIC~2010 HTTP dataset -- under both within-dataset and cross-dataset threshold-transfer protocols. Correcting a session-construction fault that let windows mix requests from different clients raises the session branch's AUC on CSIC~2010 from 0.63 to 0.97, but the branch produces no score at all on datasets where clients rarely issue more than one or two requests, showing that its benefit is conditional on genuine session structure being present in the traffic. Fusing the two branches is not uniformly beneficial: on CSIC~2010, weighting fully toward the session branch outperforms every blended fusion weight tested. A decision threshold calibrated once on normal CSIC~2010 traffic and transferred unchanged to three attack-heavy target datasets raises fused F1 by roughly five to six times over a threshold tuned on each target dataset directly (for example, 0.12 to 0.67 on Access Eval Mix 2000), because those target datasets contain too much attack traffic to yield a meaningful self-calibrated, normal-traffic threshold. The framework also achieves a substantially lower false positive rate than a labelled Random Forest baseline at comparable or better AUC than unsupervised Isolation Forest and One-Class SVM baselines, and it processes requests fast enough for near-real-time use, at about 630{,}000 lines per second in batch mode. These results indicate that how session structure and detection thresholds are built and calibrated can matter as much as the choice of underlying model.}
