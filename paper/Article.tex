...
\begin{table}[H]
\caption{Branch ablation: AUC and F1 (with 2{,}000-resample bootstrap 95\% CI) by dataset, using the corrected session model.}
\label{tab:ablation}
\begin{adjustwidth}{-\extralength}{0cm}
\begin{tabularx}{\fulllength}{lCCCCCC}
\toprule
\textbf{Dataset} & \textbf{Content AUC} & \textbf{Session AUC} & \textbf{Fused AUC} & \textbf{Content F1 [95\% CI]} & \textbf{Session F1 [95\% CI]} & \textbf{Fused F1 [95\% CI]} \\
\midrule
CSIC 2010 (held-out) & 0.70 & 0.98 & 0.76 & 0.11 [0.10, 0.11] & 0.12 [0.12, 0.13] & 0.11 [0.11, 0.12] \\
Access Eval Mix 2000 & 0.69 & --- & 0.69 & 0.11 [0.09, 0.13] & --- & 0.11 [0.09, 0.13] \\
Access Eval Small 500 & 0.71 & --- & 0.71 & 0.12 [0.08, 0.16] & --- & 0.12 [0.08, 0.16] \\
Nginx JSON Eval 800 & 0.63 & --- & 0.63 & 0.09 [0.06, 0.13] & --- & 0.09 [0.06, 0.13] \\
Access Attacks 200 & 0.48 & --- & 0.48 & 0.10 [0.05, 0.17] & --- & 0.10 [0.05, 0.17] \\
Access Mixed 500 & 0.60 & --- & 0.60 & 0.13 [0.08, 0.18] & --- & 0.13 [0.08, 0.18] \\
Access Small Benign & 0.63 & --- & 0.63 & 0.16 [0.06, 0.28] & --- & 0.16 [0.06, 0.28] \\
Access Super Long Session 2000 & n/a & n/a & n/a & 0.00 & 0.00 & 0.00 \\
\bottomrule
\end{tabularx}
{\footnotesize\textit{Note: the CSIC~2010 row is scored on the 32{,}265-row held-out split described in Section~\ref{sec5.1} (7{,}200 benign requests never used to train the session model, plus all 25{,}065 attack requests), not on the full 61{,}065-row file, since the session model is trained on part of CSIC~2010's benign traffic. F1 confidence intervals are computed by the same bootstrap procedure as Table~\ref{tab:bootstrap_ci} (\texttt{scripts/bootstrap\_ci.py}), resampling each dataset's saved per-request scores and recomputing F1 at that branch's already-reported, self-calibrated 95th-percentile threshold; Access Super Long Session 2000 is excluded because it contains only one class, so F1 is fixed at 0 with no meaningful interval.}\par}
\end{adjustwidth}
\end{table}
\unskip
...
\begin{table}[H]
\caption{Self-calibrated vs. transferred threshold, on the three target datasets, with 2{,}000-resample bootstrap 95\% CI on each F1.}
\label{tab:threshold_transfer}
\begin{adjustwidth}{-\extralength}{0cm}
\scriptsize
\begin{tabularx}{\fulllength}{llCCCCC}
\toprule
\textbf{Target Dataset} & \textbf{Branch} & \textbf{AUC} & \textbf{Self F1 [95\% CI]} & \textbf{Self FPR} & \textbf{Transf. F1 [95\% CI]} & \textbf{Transf. FPR} \\
\midrule
Access Eval Mix 2000 & Content & 0.69 & 0.11 [0.09, 0.13] & 0.02 & 0.50 [0.48, 0.53] & 0.04 \\
Access Eval Mix 2000 & Fused & 0.69 & 0.11 [0.09, 0.13] & 0.02 & 0.69 [0.67, 0.71] & 0.24 \\
Access Eval Small 500 & Content & 0.71 & 0.12 [0.08, 0.16] & 0.00 & 0.48 [0.42, 0.53] & 0.00 \\
Access Eval Small 500 & Fused & 0.71 & 0.12 [0.08, 0.16] & 0.00 & 0.67 [0.62, 0.71] & 0.16 \\
Nginx JSON Eval 800 & Content & 0.63 & 0.09 [0.06, 0.13] & 0.05 & 0.45 [0.41, 0.50] & 0.11 \\
Nginx JSON Eval 800 & Fused & 0.63 & 0.09 [0.06, 0.13] & 0.05 & 0.65 [0.62, 0.68] & 0.32 \\
\bottomrule
\end{tabularx}
{\footnotesize\textit{Note: Self F1 CIs are computed as in Table~\ref{tab:ablation}, resampling each target dataset's own saved scores at its self-calibrated threshold. Transferred F1 CIs (\texttt{scripts/bootstrap\_transfer\_f1.py}) resample the same saved scores but apply the frozen CSIC~2010-calibrated threshold (fixed across resamples, not recomputed) used for the transferred point estimate. In every case, the two intervals do not overlap, so the transferred-threshold F1 improvement is not an artefact of sampling variance at these sample sizes.}\par}
\end{adjustwidth}
\end{table}
\unskip
...
\dataavailability{The CSIC~2010 HTTP dataset is publicly available from the Information Security Institute of CSIC (Spanish National Research Council). The NASA Kennedy Space Center HTTP server logs used to train the content branch (Section~\ref{sec4.4}) are publicly available from the Internet Traffic Archive. The self-generated Apache- and Nginx-style log datasets, the trained models, scalers, and score outputs used to produce the results in Section~\ref{sec5}, and the analysis scripts used throughout this paper (including the bootstrap confidence-interval and threshold-transfer scripts) will be deposited in a public GitHub repository, archived via Zenodo for a permanent, citable DOI, upon acceptance; in the interim they are available from the corresponding author upon reasonable request.}
